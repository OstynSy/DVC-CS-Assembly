...
;  Did program produce correct results? kinda?
...
;  pop, and push, function, detecting overflow, use of param, arrays, 
;  
;  Short description of what program does:
;
;   add, mul, sub, div, mod, signed numbers together and dectects overflow
;     
;  *************************************************************
...
Main PROC
    ...
    mov     ebp, 0                  ;ebp is beginning of array for operators
...
LoopTop:
    ...
    mov     eax, operand1[esi]      ;copy the dword at [operand1 + esi] into eax
    call    DspDword                ;print signed numbers

    mPrtStr space                   ;prints space
    mov     al, operators[ebp]      ;copy the dword at [operators + ebp] into eax
    call    WriteChar               ;print signed numbers
    mPrtStr space                   ;prints space

    mov     eax, operand2[esi]      ;copy the dword at [operand1 + esi] into eax
    call    DspDword                ;print signed numbers

    mPrtStr equal                   ;prints equal sign

    push    operand2[esi]           ;push operand 2 onto stack
    push    operand1[esi]           ;push operand 1 onto stack

    mov     al, operators[ebp]      ;copy the dword at [operators + ebp] into eax
    cmp     al, '-'                 ;if al = -
    je      subtraction             ;jumps to subtraction

    cmp     al, '+'                 ;if al = +
    je      addition                ;jumps to addition

    cmp     al, '*'                 ;if al = *
    je      multiplication          ;if equal jump to multiplication

    call    doDiv                   ;calls do div function
    jmp     next                    ;jumps to next

subtraction:
    call    doSub                   ;calls do sub function
    jo      overflow                ;jumps to overflow
    jmp     next                    ;jumps next

addition:
    call    doAdd                   ;calls do add function
    jo      overflow                ;jumps to overflow
    jmp     next                    ;jumps to next

multiplication:
    call    doMult                  ;calls do mult function
    jo      multoverflow            ;jumps to multiplication overflow

next:              
    call    DspDword                ;prints out signed numbers
    jmp     next1                   ;jumps to next1

multoverflow:
    mPrtStr multOF                  ;prints mult overflow
    jmp     next1                   ;jumps next

overflow:
    test    eax,eax                 ;tests for signed or unsigned
    js      positiveof              ;if signed jump positive of
    mPrtStr negOF                   ;print neg of
    jmp     next1                   ;jump next

positiveof:
    mPrtStr posOF                   ;prints positive overflow message

next1:
    add     ebp, 1                  ;increments ebp by 1
    add     esi, 4                  ;ebx contains the offset from the beginning of the array. 
                                    ;add 4 to esi so that operand + esi points to the 
...
DspDword proc
    ...
    or       eax, eax               ;or eax with itself to check for sign flag
    jns      positive               ;check signed flag to jump or not

    mov      edx, eax               ;stores eax into edx
    mov      eax, "-"               ;stores "-" into eax
    call     WriteChar              ;writes char
    mov      eax, edx               ;restores the value of eax
    neg      eax                    ;does 2 complement to change the number positive
...
; Remove ps
    ...
    mov      eax, $parm1            ;stores operand 1 into eax
    neg      $parm2                 ;twos complement of op2
    add      eax, $parm2            ;add eax and op2

    pop      ebp                    ;return ebp to its original value
    ret      8                      ;return
doSub ENDP
...
;
    ...
    mov      ebp,esp                ;EBP= address of original EBP on stack
    mov      eax, $parm1            ;stores operand 1 into eax
    imul     $parm2                 ;signed multiplication 
...
; Remove parameters by using ret 8 rather than just ret at the end of this function
    ...
    mov      eax, $parm1            ;stores operand 1 into eax
    cdq                             ;converts dwor to qword
    idiv     $parm2                 ;signed division

    je       done1                  ;if equal then jump to done1

    mov      eax, edx               ;if not equal then take the remainder and store into eax
